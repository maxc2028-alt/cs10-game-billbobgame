...
class GameView(arcade.View):
    ...
    def reset_round(self) -> None:
        ...
        # Trash positions — on the GROUND in front of the building, clearly visible
        relative_positions = [
            (-70, 10), (-35, 8), (0, 12), (40, 6), (70, 10), (20, 8)
        ]
        for rel_x, rel_y in relative_positions:
            self.trash_spots.append(TrashSpot(building_center_x + rel_x, base_y + rel_y))

        # Friends stand ON the ground next to the building — not floating
        for i in range(3):
            friend_name = FRIEND_NAMES[i % len(FRIEND_NAMES)]
            # Position friends spread around building, grounded at street level
            side_offset = (i - 1) * 150
            fx = building_center_x + side_offset
            # Ground level: base_y is the building bottom, friends stand just in front
            fy = base_y + 18  # feet at ground level
    ...
    def draw_trash(self, trash: TrashSpot, pulse: float = 0.0) -> None:
        """Draw highly visible trash with glow effect."""
        x, y = trash.x, trash.y

        # Pulsing glow ring — makes trash super obvious
        glow_alpha = int(140 + 80 * math.sin(pulse + trash.pulse))
        glow_size = 28 + 4 * math.sin(pulse + trash.pulse)
        arcade.draw_circle_filled(x, y, glow_size, (255, 200, 50, glow_alpha))

        # Downward arrow above trash so player knows to collect
        arcade.draw_triangle_filled(x - 8, y + 40, x + 8, y + 40, x, y + 26, (255, 80, 80, 200))
        arcade.draw_triangle_filled(x - 6, y + 50, x + 6, y + 50, x, y + 38, (255, 80, 80, 160))
